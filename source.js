...
var createroom = function() {
    ...
    var roomform = `<br><div class="container">
        <div id="Room` + id + `" class="card text-center">
            ...
            <div class="card-body">
                <button onclick="createswitchboard(this);" type="button" class="btn btn-outline-primary">Add Switchboard </button>
            </div>
        </div>
    </div>`;
...
var createswitchboard = function(currentroom) {
    var switchboard = `<br><div class="container">
    <div id="Switchboard` + switchboardid + `" class="card text-center">
        <div class="card-header">
    ...
    $(currentroom).before(switchboard);
    switchboardid += 1;
}
...
function finish() {
    ...
    $('*[id*=Room]:visible').each(function() {
        ...
        switchboarddetails = [];
        //  console.log(this.id);
        $(this).find('*[id*=Switchboard]:visible').each(function() {
            ...
            var Switchboardframe = {
                "Switchboard": this.id,
                "Roomdevices": devicedetails
            }

            switchboarddetails.push(Switchboardframe);

        });

        var roomframe = {
            "Room": this.id,
            "Roomswitches": switchboarddetails
        }
    ...
    for (var all in finalsitedetails) {
        ...
        for (var all2 in finalsitedetails[all]["Roomswitches"]) {
            row.push("", finalsitedetails[all]["Roomswitches"][all2]["Switchboard"], "Category", "Product", "Price Per Unit", "Count", "Total Price");
            ws_data.push(row);
            row = [];
            for (var all3 in finalsitedetails[all]["Roomswitches"][all2]["Roomdevices"]) {
                totalsum = totalsum + finalsitedetails[all]["Roomswitches"][all2]["Roomdevices"][all3]["totalPrice"];
                row.push("", all3, finalsitedetails[all]["Roomswitches"][all2]["Roomdevices"][all3]["category"], finalsitedetails[all]["Roomswitches"][all2]["Roomdevices"][all3]["product"], finalsitedetails[all]["Roomswitches"][all2]["Roomdevices"][all3]["count"], finalsitedetails[all]["Roomswitches"][all2]["Roomdevices"][all3]["ppu"], finalsitedetails[all]["Roomswitches"][all2]["Roomdevices"][all3]["totalPrice"]);
                ws_data.push(row);
                row = [];
            }
            row = ["", ""];
